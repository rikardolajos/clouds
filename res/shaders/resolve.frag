...
vec4 cloud_sampling(vec3 v, vec4 color, float delta) {
	...
	vec3 p = vec3(texture(perlin1, v.xyz / 50).b);
	vec3 w = vec3(texture(worley, v.xyz / 150).r);
	value = color * vec4((p + 0.5) * (w - 0.3) * 0.5, 1.0) * delta * 0.2;
...
vec4 cast_ray_cube(vec3 origin, vec3 dir) {
	...
	vec4 cloud_dark = vec4(vec3(-0.58), 1.0);
	...
	float length_inside = smoothstep(20, 40, length(inside_end - inside_start));
